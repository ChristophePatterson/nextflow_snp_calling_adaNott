...
# Repadapt run name
run_version="RepAdapt_NF_GasAcu_fw_2026-03-26"
...
bcftools query -l output/$run_version/final_variants.vcf.gz > complete_NF_samps.txt
...
outputdir="output/$run_version/$runname"
mkdir $outputdir

# 1) vcf
cp output/$run_version/final_variants.vcf.gz $outputdir/$runname.vcf.gz
# 2) Genome seq
...
# 5) Gene depth (No longer required)
# ------
# 6) Gene depth denovo
cp output/$run_version/combined_genes.tsv $outputdir/$runname.depth_gene_denovo.gz

# 7) Window depth
cp output/$run_version/combined_windows.tsv $outputdir/$runname.depth_gene_5Kb.gz

# 8) Whole genome depth
gzip -c output/$run_version/combined_wg.tsv > $outputdir/$runname.depth_wg.gz

# Meta data
cp RepAdapt_rawg0173_Gasterosteus_aculeatus_Patterson_metadata.txt $outputdir/RepAdapt_rawg0173_Gasterosteus_aculeatus_Patterson_metadata.txt 
